...
import {addTodolistAC, removeTodolistAC} from "../../TodoList/Todo/todoSlice";
import {AppDispatchType, RootStateType} from "../../../../store/store";
import {TaskType, todoApi, UpdateModelType} from "../../../../api/todo-api";
...
export const addTaskTC = createAsyncThunk<unknown, { todoId: string, title: string }, AsyncThunkConfig>(
    'tasks/addTaskTC',
    async (payload, {dispatch}) => {
        dispatch(setLoading({status: 'loading'}))
        try {
            const res = await todoApi.createTask(payload.todoId, payload.title)
...
export const deleteTaskTC = createAsyncThunk<unknown, { todoId: string, taskId: string }, AsyncThunkConfig>(
    ...
    async (payload, {dispatch}) => {
        ...
        try {
            const res = await todoApi.deleteTask(payload.todoId, payload.taskId)
            if (res.data.resultCode === 0) {
...
export const updateTaskTC = createAsyncThunk<unknown, updateTaskTCType, AsyncThunkConfig>(
    ...
    async (payload, {dispatch, getState}) => {
        ...
        if (task) {
            ...
            try {
                const res = await todoApi.updateTask(payload.todoId, payload.taskId, model)
                if (res.data.resultCode === 0) {
